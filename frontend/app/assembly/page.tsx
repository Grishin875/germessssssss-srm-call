...
export default function AssemblyPage() {
  ...
  const getComponents = (name: string) => recipes.filter(r => r.product_name === name && r.production_type === PROD_TYPE);
    ...
    <AppLayout>
      <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <h1 style={{ margin: 0 }}>Монтаж — ручная сборка</h1>
            <p style={{ margin: "4px 0 0", color: "var(--text-muted)", fontSize: 13 }}>Установка компонентов вручную после СМД</p>
          </div>
        ...
        {fetching ? (
          ...
          <Card><div style={{ textAlign: "center", padding: 40, color: "var(--text-muted)" }}>Нет активных партий монтажа</div></Card>
        ) : (
              ...
              return (
                <Card key={b.batch_id}>
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
                    <div style={{ width: 4, alignSelf: "stretch", borderRadius: 4, background: "#0ea5e9", flexShrink: 0 }} />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {comps.length > 0 && (
                        <button onClick={() => setExpanded(isExpanded ? null : b.batch_id)}
                          style={{ background: "none", border: "none", cursor: "pointer", fontSize: 12, color: "#0ea5e9", fontWeight: 600, padding: 0, marginBottom: 8 }}>
                          {isExpanded ? "▲ Скрыть" : `▼ Компоненты для сборки (${comps.length})`}
                        </button>
                      )}
                      {isExpanded && (
                        <div style={{ overflowX: "auto", marginBottom: 10 }}>
                          <table style={{ borderCollapse: "collapse", fontSize: 12, width: "100%" }}>
                            <thead><tr>{["Компонент","Норма на 1 шт","Всего нужно"].map(h=><th key={h} style={{ textAlign:"left",padding:"2px 8px",color:"var(--text-muted)",fontWeight:600 }}>{h}</th>)}</tr></thead>
                            <tbody>
                              {comps.map(c => (
                                <tr key={c.id}>
                                  <td style={{ padding: "3px 8px", fontWeight: 500 }}>{c.component_name}</td>
                                  <td style={{ padding: "3px 8px" }}>{c.norm}</td>
                                  <td style={{ padding: "3px 8px", color: "#0ea5e9", fontWeight: 600 }}>{(c.norm * b.planned_qty).toFixed(0)}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
              ...
              {otkReady.map(b => (
                <Card key={b.batch_id}>
                  <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                    ...
                    <Button size="sm" onClick={() => submitToOtk(b)} loading={otkSubmitting === b.batch_id}>
                      Сдать на ОТК
                    </Button>
                  </div>
                </Card>
      ...
      <Modal open={!!startModal} onClose={() => setStartModal(null)} title={`Запустить ${startModal?.batch_id}`}
        ...
        <div><label>Табельный номер оператора *</label>
          <input value={operatorId} onChange={e => setOperatorId(e.target.value)} list="op-list2" placeholder="Номер сотрудника" />
          <datalist id="op-list2">{operators.map(o => <option key={o.employee_id} value={o.employee_id}>{o.name}</option>)}</datalist>
        </div>
      </Modal>

      <Modal open={!!pauseModal} onClose={() => setPauseModal(null)} title={`Пауза — ${pauseModal?.batch_id}`}
        ...
        <div className="space-y-3">
          <div><label>Произведено за смену</label><input type="number" value={pauseQty} onChange={e => setPauseQty(e.target.value)} min="0" /></div>
          <div><label>Комментарий</label><textarea value={pauseComment} onChange={e => setPauseComment(e.target.value)} rows={2} /></div>
        </div>
      </Modal>
    </AppLayout>
  );
